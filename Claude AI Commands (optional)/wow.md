...
- **Map canvas taint (12.0.0+)**: Addon frames on `WorldMapFrame:GetCanvas()` cause taint. Use `nil` frame names, `EnableMouse(false)`. For map pins, use `CreateUnsecuredRegionPoolInstance` (not `CreateFramePool`) and pre-register in `WorldMapFrame.pinPools`. `AddDataProvider` inserts tainted keys; prefer manual pin lifecycle. Never `hooksecurefunc` on WorldMapFrame methods; use `EventRegistry` callbacks (`WorldMapOnShow`, `WorldMapOnHide`, `MapCanvas.MapSet`) via `RegisterCallbackWithHandle` (not `RegisterCallback` with addon objects as owner keys -- tainted keys leak through `secureexecuterange`). Pin `OnReleased` fires before `OnLoad` on first acquire — must be nil-safe. No `MapCanvasPinTemplate` exists; use plain frames with mixin. Override `SetPassThroughButtons = function() end` on pin mixins. Use `OpenWorldMap(mapID)` instead of `ShowUIPanel` + `SetMapID`.
- **GameTooltip taint (12.0.0+)**: `SetOwner()` does NOT hide `ItemTooltip` — always call `GameTooltip.ItemTooltip:Hide()` after `SetOwner()` to prevent stale state from tainting dimensions via `GameTooltip_CalculatePadding`. Avoid `GameTooltip_ShowProgressBar` (calls `GetWidth()` which returns secret). Wrap `GameTooltip_AddQuestRewardsToTooltip` with `pcall()`. **Definitive fix:** use a private tooltip (`CreateFrame("GameTooltip", "MyAddonTooltip", UIParent, "GameTooltipTemplate,BackdropTemplate")`) instead of the shared GameTooltip.
