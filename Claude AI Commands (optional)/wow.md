...
- **Map canvas taint (12.0.0+)**: Addon frames on `WorldMapFrame:GetCanvas()` cause taint. Use `nil` frame names, `EnableMouse(false)`. For map pins, use `CreateUnsecuredRegionPoolInstance` (not `CreateFramePool`) and pre-register in `WorldMapFrame.pinPools`. `AddDataProvider` inserts tainted keys; prefer manual pin lifecycle. Pin `OnReleased` fires before `OnLoad` on first acquire — must be nil-safe. No `MapCanvasPinTemplate` exists; use plain frames with mixin. Override `SetPassThroughButtons = function() end` on pin mixins. Use `OpenWorldMap(mapID)` instead of `ShowUIPanel` + `SetMapID`.
